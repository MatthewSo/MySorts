...
public class SelectionSort {
	...
	int curr = 0;
	boolean n= false;
	while (curr < data.size() - 1){
	    n = false;
	    for (int i=curr;i<data.size();i++){
	...
	int curr = 0;
	boolean n= false;
	while (curr < output.size() - 1){
	    n = false;
	    for (int i=curr;i<output.size();i++){
		if (output.get(i).compareTo(output.get(min))<0){
		    min = i;
		    n = true;
		}
